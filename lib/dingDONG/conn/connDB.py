...
from dingDONG.misc.enums            import eConn, eSql, eJson
...
DEFAULTS = {
            ...
            eConn.types.LITE: {   eConn.defaults.DEFAULT_TYPE:'varchar(100)',eConn.defaults.TABLE_SCHEMA:None,
                                  eConn.defaults.COLUMNS_NULL:'Null', eConn.defaults.UPDATABLE:False}
           }
...
class connDb (baseConnBatch):
    def __init__ (self, propertyDict=None, connType=None, connName=None, connIsTar=None,
        ...
        p("CONNECTED, DB TYPE: %s, %s" % (self.connType, objName, ), "ii")
    ...
    def create(self, stt=None, objName=None, addIndex=None):
        ...
        if config.DING_ADD_OBJECT_DATA:
            if newHistoryTable and len (newHistoryTable)>0:
                ...
                if len (columns)>0:
                    sql = setSqlQuery().getSql(conn=self.connType, sqlType=eSql.TABLE_COPY_BY_COLUMN, tableName=tableName,tableSchema=tableSchema, srcTableName=newHistoryTable, columns=columns )
                    self.exeSQL(sql=sql)
    ...
    def cloneObject(self, newStructure, tableName, tableSchema=None):
        ...
        for col in newStructure:
            colAlias = newStructure[col][eJson.stt.ALIACE] if eJson.stt.ALIACE in newStructure[
                col] else None
            colType = newStructure[col][eJson.stt.TYPE] if eJson.stt.TYPE in newStructure[
    ...
    def getStructure(self, tableName=None, tableSchema=None, sqlQuery=None):
        ...
        for col in retStructure:
            ...
            if eJson.stt.ALIACE in finallStructure[col] and finallStructure[col][
                eJson.stt.ALIACE] is not None:
                finallStructure[col][eJson.stt.ALIACE] = finallStructure[col][eJson.stt.ALIACE].replace(
                    pre, "").replace(pos, "")
                    ...
                    for colTbl in tableColL:
                        if colTbl.lower() in colSName.lower():
                            ret[colTarName] = {eJson.stt.SOURCE: tableColL[colTbl],
                                               eJson.stt.TYPE: tableStrucure[tableColL[colTbl]][
                                                   eJson.stt.TYPE]}
                            isFound = True
                            break
    ...
    def truncate(self, tableName=None, tableSchema=None):
        ...
        self.exeSQL(sql=sql)
        p("TYPE:%s, TRUNCATE TABLE:%s" % (self.connType, self.connTbl),"ii")
        ...
        if tarToSrc and len (tarToSrc)>0:
            ...
            for i,col in  enumerate (tarToSrc):
                ...
                if eJson.stt.SOURCE in tarToSrc[col] and tarToSrc[col][eJson.stt.SOURCE]:
                    srcColumnName = tarToSrc[col][eJson.stt.SOURCE].replace(pre, "").replace(pos, "").lower()
                ...
                elif eJson.stt.EXECFUNC in tarToSrc[col] and len(tarToSrc[col][eJson.stt.EXECFUNC])>0:
    ...
    def load(self, rows, targetColumn):
        ...
        if totalRows == 0:
            p("THERE ARE NO ROWS")
            return
        tblFullName = "%s.%s" %(self.defaultSchema, self.connTbl) if self.defaultSchema else self.connTbl
        execQuery = "INSERT INTO %s" % (tblFullName)
        pre, pos = self.columnFrame[0], self.columnFrame[1]
        colList = []
        colInsert = []
        ## Compare existint target strucutre
        tarStrucutre = self.getStructure(tableName=self.connTbl, tableSchema=self.defaultSchema, sqlQuery=None)
        ...
        try:
            ...
            self.connDB.commit()
            p('LOAD %s into target: %s >>>>>> ' % (str(totalRows), self.connTbl), "ii")
